...
df_allocation_desinvest = df_allocations_client[[
    'Support', 'Code ISIN', 'Type', 'SRI', 'Encours en €', '+/- value (en %))']]
Montant_total = df_allocation_desinvest['Encours en €'].sum()
df_allocation_desinvest['Ratio désinvestissement'] = '0.'
# df_allocation_desinvest['Montant désinvesti'] = '0.'
df_allocation_desinvest = df_allocation_desinvest[[
    'Support',  'Type', 'Ratio désinvestissement',
    'Encours en €', '+/- value (en %))', 'SRI', 'Code ISIN']]

# expanders creattion
expander_desinvest = st.expander("Désinvestissement")
expander_invest = st.expander("Choix des supports")
expander_new_alloc = st.expander("Ré-investissement")

# Desinvestissment
expander_desinvest.write("Fonds à désinvestir")
df_allocation_desinvest_mod = expander_desinvest.data_editor(df_allocation_desinvest,
                                                             hide_index=True,
                                                             disabled=['Support', 'Code ISIN', 'Type', 'SRI',
                                                                       'Encours en €', '+/- value (en %))'],
                                                             column_config={
                                                                 "Ratio désinvestissement": st.column_config.NumberColumn(
                                                                     "Ratio désinvestissement",
                                                                     help="Ratio de l'en cours à désinvestir",
                                                                     min_value=0,
                                                                     max_value=100,
                                                                     step=1,
                                                                     format="%d %%",
...
df_allocation_desinvest_mod['Montant désinvesti'] = (
    df_allocation_desinvest_mod['Ratio désinvestissement'].astype(float) *
    df_allocation_desinvest_mod['Encours en €'].astype(float) / 100)

Total_reinvestissement = df_allocation_desinvest_mod['Montant désinvesti'].sum(
)
expander_desinvest.metric(
    "Montant à réinvestir: ", Total_reinvestissement)

# Ré-investissement
expander_invest.write("Fonds à ré-invertir")
...
if (button_MAJ_new_alloc):
    ...
    df_fonds_desinvestis = df_new_alloc[df_new_alloc['Ratio désinvestissement'].astype(
        float) != 0]
    df_fonds_desinvestis['Ratio désinvestissement'] = df_new_alloc['Ratio désinvestissement'].astype(
        float)
    ...
    df_new_alloc['Encours en €'] = df_new_alloc['Encours en €'] * \
        (1. -
            df_new_alloc['Ratio désinvestissement'].astype(float)/100)
    ...
    types_list = df_allocation_desinvest["Type"].unique(
    ).tolist() + df_new_alloc['Type'].unique().tolist()
    c = dict(zip(types_list, px.colors.qualitative.G10))
    supports_list = df_allocation_desinvest["Support"].unique(
    ).tolist() + df_new_alloc['Support'].unique().tolist()
